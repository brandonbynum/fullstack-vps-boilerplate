...
echo "=== Full-Stack App Deployment (Simple) ==="
...
podman stop fullstack-postgres fullstack-backend fullstack-frontend fullstack-nginx 2>/dev/null || true
podman rm fullstack-postgres fullstack-backend fullstack-frontend fullstack-nginx 2>/dev/null || true

# Create network if it doesn't exist
podman network rm fullstack-app_internal 2>/dev/null || true
podman network create fullstack-app_internal

# Check if postgres volume exists, if not create it
